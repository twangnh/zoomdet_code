...
class BaseDenseHead(BaseModule, metaclass=ABCMeta):
    ...
    def loss_and_predict(
        ...
        proposal_cfg: Optional[ConfigDict] = None) -> Tuple[dict, InstanceList]:
    ...
    def predict(self,
        ...
        outs = self(x)
        grid = None
        if batch_data_samples[0].get('grid') is not None:
            grid = batch_data_samples[0].grid

        predictions = self.predict_by_feat(
            *outs, batch_img_metas=batch_img_metas, rescale=rescale, grid=grid)
        return predictions
