...
class CocoMetric(BaseMetric):
    ...
    def __init__(self,
        ...
        if ann_file is not None:
            with get_local_path(
                ...
                #     temp = json.load(f)
                # anns = temp['annotations']
                # temp['annotations'] = [a for a in anns if a['area']<300]
                # filtered_json_path = local_path.replace('val.json','val_area_lessthan_300.json')
                # with open(filtered_json_path, 'w') as f:
                #     json.dump(temp, f)
                # self._coco_api = COCO(filtered_json_path)
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        # from .tide_dataset import COCO, COCOResult
        #
        # tide = TIDE()
        #
        # tide.evaluate(COCO(self.ann_file), COCOResult(result_files['bbox']),
        ...
        for metric in self.metrics:
            ...
            coco_eval.params.areaRng.append([0, 300])
            coco_eval.params.areaRngLbl.append('ex_small')
            ...
            if metric == 'proposal':
                ...
                if metric_items is None:
                    metric_items = [
                        ...
                        'AR_m@1000', 'AR_l@1000', 'AR_es@1000'
                    ]
                ...
                if metric_items is None:
                    metric_items = [
                        'mAP', 'mAP_50', 'mAP_75', 'mAP_s', 'mAP_m', 'mAP_l', 'mAP_es'
                    ]
